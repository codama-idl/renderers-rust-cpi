...
Thanks for your interest in contributing to `@codama/renderers-rust-cpi`.
...
`main` makes the release workflow open (or update) a release PR, and
merging that release PR publishes the package to npm.

Major releases follow the ecosystem-wide process described in the spec
repository's
[RELEASING.md](https://github.com/codama-idl/spec/blob/HEAD/RELEASING.md)
(branch model, npm dist-tag conventions, release candidates, the bake window,
and the cross-repo release train). This repository is pre-1.0, so nothing
special applies yet: its first `N.x` maintenance branch will simply appear at
its first major cut.
